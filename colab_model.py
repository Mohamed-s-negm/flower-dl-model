...
class Custom_Model:
    def __init__(self, model_type='scratch', device=None, num_classes=None, model_path=None,
        ...
        self.model_path = model_path or f"model_{model_type}.pth"
        ...
        if self.model_type == 'scratch':
            ...
            if self.model_type == 'feature_extraction':
                ...
                for name, param in vgg.features.named_parameters():
                  if int(name.split('.')[0]) >= 10:  # conv4_3 and up
                    param.requires_grad = True
...
if __name__ == "__main__":
    ...
    for model_type in ['scratch', 'feature_extraction', 'fine_tuning']:
        print(f"\n\n==== Training {model_type.upper()} Model ====")
        model = Custom_Model(model_type=model_type, num_classes=5, model_path=f"model_{model_type}.pth")
        model.prepare(data_path)
        model.set_model()
        model.train()
        val_acc = model.evaluate()
        test_acc = model.test()
        results[model_type] = {'val_acc':val_acc, 'test_acc':test_acc}
        print(f"{model_type.upper()} val_accuracy: {val_acc:.4f}, test_accuracy: {test_acc:.4f}")
        model.plot_confusion_matrix()

    print("\n\n========== Final Results ==========")
    for k, v in results.items():
        print(f"{k.upper()} model - Val Acc: {v['val_acc']:.4f}, Test Acc: {v['test_acc']:.4f}")
